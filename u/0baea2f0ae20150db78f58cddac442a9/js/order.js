...
import { query } from "../../../js/firebase_xp";
...
navBtns.forEach((navBtn, index) => {
    navBtn.addEventListener('click', (e) => {
        ...
        const cg = query(collection)
        for (let x = 0; x < 3; x++) {
            const clone = asideTemplate.content.cloneNode(true);
            // clone.setAttribute('data-sn', x);
            clone.querySelector('.usr').style.backgroundColor = ['#fab500','#1a73e8','#009578'][x];
